		...
		register_nav_menus(
			array(
				...
				'menu2' => __( 'Menu 2','atahualpa' ),
                                'menu3' => __( 'Menu 3','atahualpa' ),
                                'menu4' => __( 'Menu 4','atahualpa' ),
                                'menu5' => __( 'Menu 5','atahualpa' ),
                                'menu6' => __( 'Menu 6','atahualpa' ),
                                'menu7' => __( 'Menu 7','atahualpa' ),
                                'menu8' => __( 'Menu 8','atahualpa' ),
                                'menu9' => __( 'Menu 9','atahualpa' ),
                                'menu10' => __( 'Menu 10','atahualpa' ),
			)
		);
	}
}
...
add_action('comment_unapproved_to_approved', 'efan_comment_approved');
 
function efan_comment_approved($comment) 
{
    if (is_email($comment->comment_author_email)) 
    {
        $post_link = get_permalink($comment->comment_post_ID);
        $title = '【审核通过】您在『' . get_bloginfo('name') . '』的评论已通过审核';
 
        $body = '<span style="color: green; font-size: 16px;"><strong>您在《<a href="' . $post_link . '" target="_blank" style="color: green;">' . get_the_title($comment->comment_post_ID) . '</a>》中发表的评论已通过审核！</strong></span><br /><br />';
        $body .= '<strong>您的评论：</strong><br />';
        $body .= strip_tags($comment->comment_content) . '<br /><br />';
        $body .= '您可以：<a href="' . get_comment_link($comment->comment_ID) . '" target="_blank">查看您的评论</a>  |  <a href="' . $post_link . '#comments" target="_blank">看看别人怎么说</a>  |  <a href="' . $post_link . '" target="_blank">查看原文</a><br /><br />';
        $body .= '欢迎再次访问『<a href="' . get_bloginfo('url') . '" target="_blank" title="' . get_bloginfo('description') . '">' . get_bloginfo('name') . '</a>』。';
        $body .= '<br /><br />此邮件为系统自动发送，请勿直接回复，<a href="' . get_bloginfo('url') . '/question" target="_blank">联系Tax1on1 Team请点击这里»</a>';
