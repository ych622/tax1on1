...
function tie_last_news_pic($order , $numberOfPosts = 12 , $cats = 1 ){
	...
	if( $order == 'random')
		$lastPosts = get_posts(	$args = array('numberposts' => $numberOfPosts, 'orderby' => 'rand', 'no_found_rows' => 1, 'category' => $cats ));
	else
		$lastPosts = get_posts(	$args = array('numberposts' => $numberOfPosts, 'no_found_rows' => 1, 'category' => $cats ));
...
function tie_last_posts($numberOfPosts = 5 , $thumb = true){
	...
	$lastPosts = get_posts('no_found_rows=1&numberposts='.$numberOfPosts);
	foreach($lastPosts as $post): setup_postdata($post);
?>
...
function tie_last_posts_cat($numberOfPosts = 5 , $thumb = true , $cats = 1){
	...
	$lastPosts = get_posts('category='.$cats.'&no_found_rows=1&numberposts='.$numberOfPosts);
	foreach($lastPosts as $post): setup_postdata($post);
?>
<li <?php tie_post_class(); ?>>
	<?php if ( function_exists("has_post_thumbnail") && has_post_thumbnail() && $thumb ) : ?>			
...
function tie_random_posts($numberOfPosts = 5 , $thumb = true){
	...
	$lastPosts = get_posts('orderby=rand&no_found_rows=1&numberposts='.$numberOfPosts);
	foreach($lastPosts as $post): setup_postdata($post);
?>
<li <?php tie_post_class(); ?>>
	<?php if ( function_exists("has_post_thumbnail") && has_post_thumbnail() && $thumb ) : ?>			
...
function tie_option_posts_per_page( $value ) {
    ...
    if ( is_home() && tie_get_option('on_home') == 'boxes' ) {
		$boxes = get_option( 'tie_home_cats' ) ;
		foreach ( $boxes as $box){
			if( !empty($box['type']) && !empty($box['number']) && !empty($box['pagi']) && $box['pagi'] == 'y' && $box['type'] == 'recent'){
				$number = $box['number'];
				break;
			}
		}
