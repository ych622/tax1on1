	...
	*/
	...
	$wl_global = array(
		...
		'bfa_loop' => array( 
			...
			'examples' => array(
				"<?php bfa_loop(); ?>" => __( 'Displays the list of posts (the WordPress "Loop") on multi post pages. 
					...
					template if no other base post format template is provided inside the brackets and the WordPress 
					<code>Blog pages show at most NN posts</code> if no count is used.', 'montezuma' ),
				...
				"<?php bfa_loop( 'postformat', 2 ); ?>" => __( 'Limits the number of posts to display. This value must be smaller
				 than the WordPress Settings value <code>Blog pages show at most NN posts</code> or it will be ignored. You must 
				 specify a post format when using this option.', 'montezuma' ),
			),
		...
		*/
		...
		'bfa_custom_query' => array( 
			'type' => 'single',	
			'examples' => array(
				'<?php bfa_custom_query(); ?>' => __( 'This function can be used to run a custom query.', 'montezuma' ),
			),
			'info' => __( 'To craft a query, go to <code>http://codex.wordpress.org//Function_Reference/query_posts</code>. Use what is inside the quotes. For example, if you see 
			<code>query_posts(\'cat=2,6,17,38\')</code> you would only enter <code>cat=2,6,17,38</code> inside the parentheses. For example <code><?php bfa_custom_query(\'cat=2,6,17,38\')</code><br /><code><?php bfa_custom_query( \'status=published&post_format=post-format-audio\'); ?></code> WARNING: improper use of this function can cause issues with your site.', 'montezuma' ),
		),		
		...
		'bfa_comments_title' => array( 
			'type' => 'single',
			'examples' => array(
				'<?php bfa_comments_title(); ?>' => __( 'Display the number of comments the post has.', 'montezuma'),
			),
			'info' => '',
		),
	...
	$wl_loop = array(
		...
		'wp_link_pages' => array( 
			...
			//'type' => 'queryarray',
			'examples' => array(
				"<?php wp_link_pages( array( 'before' => __('<p class=\"post-pagination\">Pages:', 'montezuma'), 'after' => '</p>' ) ); ?>" => __( 'Displays page-links for paginated posts.', 'montezuma' ),
			),
