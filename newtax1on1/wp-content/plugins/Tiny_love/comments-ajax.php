<?php
/*
 * haozi / hao.chen@qq.com / www.daqianduan.com / t.qq.com/daqianduan &&
 * weibo.com/daqianduan / 2011.11.06 Theme D4 => Ajax评论配置
 */

/**
 * WordPress 內置嵌套評論專用 Ajax comments >> WordPress-jQuery-Ajax-Comments v1.3 by
 * Willin Kan.
 *
 ...
 */
require (dirname ( __FILE__ ) . '/../../../wp-load.php'); // 此
                                                             // comments-ajax.php
                                                             // 位於主題資料夾,所以位置已不同
nocache_headers ();
...
if (! comments_open ( $comment_post_ID )) {
	...
	err ( __ ( 'Sorry, comments are closed for this item.' ) ); // 將 wp_die
	                                                            // 改為錯誤提示
} elseif ('trash' == $status) {
...
$edit_id = (isset ( $_POST ['edit_id'] )) ? $_POST ['edit_id'] : null; // 提取
                                                                       // edit_id
                                                                       // If
                                                                       // the
                                                                       // user
...
if ($user->ID) {
	...
	if (get_option ( 'comment_registration' ) || 'private' == $status)
		//err ( __ ( 'Sorry, you must be logged in to post a comment.' ) ); // 將
		 err ( __ ( '<img alt="" style="vertical-align: middle;" src="/wp-admin/images/wpspin_dark.gif"> <strong>抱歉</strong>：，发表评论您须登录。' ) );                // wp_die
			                                                                  // 改為錯誤提示
}
$comment_type = '';
if (get_option ( 'require_name_email' ) && ! $user->ID) {
	if (6 > strlen ( $comment_author_email ) || '' == $comment_author)
		//err ( __ ( 'Error: please fill the required fields (name, email).' ) ); // 將
		err ( __ ( '<strong>错误</strong>：请填写必填项目（姓名和电子邮件地址）。' ) );   // wp_die
			                                                                        // 改為錯誤提示
	elseif (! is_email ( $comment_author_email ))
		//err ( __ ( 'Error: please enter a valid email address.' ) ); // 將 wp_die
		err ( __ ( '<strong>错误</strong>：请输入有效的电子邮件地址。' ) ); // 將 wp_die
			                                                             // 改為錯誤提示
}
if ('' == $comment_content)
	err ( __ ( '<strong>错误</strong>：请输入评论内容。' ) ); // 將 wp_die 改為錯誤提示
		                                                // 增加: 錯誤提示功能
function err($ErrMsg) {
	header ( 'HTTP/1.1 405 Method Not Allowed' );
	echo $ErrMsg;
...
if ($wpdb->get_var ( $dupe )) {
	...
	err ( __ ( '<strong>警告</strong>：检测到重复评论，您似乎已经提交过这条评论了！' ) );
}
// 增加: 檢查評論太快功能
if ($lasttime = $wpdb->get_var ( $wpdb->prepare ( "SELECT comment_date_gmt FROM $wpdb->comments WHERE comment_author = %s ORDER BY comment_date DESC LIMIT 1", $comment_author ) )) {
	...
	if ($flood_die) {
		err ( __ ( '<strong>提示</strong>：您唧唧歪歪的速度太快了，坐下来喝杯茶吧。' ) );
	}
}
